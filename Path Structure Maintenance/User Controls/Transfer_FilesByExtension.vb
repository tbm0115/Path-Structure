...
Public Class Transfer_FilesByExtension
  ...
  Public Sub New(ByVal CurrentPath As String)
    ...
    For Each fol As XmlElement In myXML.SelectNodes("//Folder")
    ...
    If _CurrentPath.Type = PathStructure.PathType.File Then
      txtFileExtension.AutoCompleteCustomSource.Add(_CurrentPath.FileInfo.Extension)
      txtFileExtension.Text = _CurrentPath.FileInfo.Extension
    ...
    If _CurrentPath.Type = PathStructure.PathType.Folder Then
      For Each fil As PathStructure In _CurrentPath.Children
        If fil.Type = PathStructure.PathType.File Then
          If Not txtFileExtension.AutoCompleteCustomSource.Contains(fil.FileInfo.Extension) Then
            txtFileExtension.AutoCompleteCustomSource.Add(fil.FileInfo.Extension)
          End If
        End If
      Next
    End If
  ...
  Private Sub btnTransfer_Click(sender As Object, e As EventArgs) Handles btnTransfer.Click
    If cmbFolder.SelectedIndex > 0 And Not String.IsNullOrEmpty(txtFileExtension.Text) Then
      Dim fold As XmlNodeList = myXML.SelectNodes("//Folder[@name='" & cmbFolder.Items(cmbFolder.SelectedIndex).ToString & "']")
      If fold.Count = 1 Then
        ...
        For Each fol As XmlNode In fold
          xpath = FindXPath(fol)
          fpath = _CurrentPath.GetURIfromXPath(xpath)
          ...
          For Each child As PathStructure In _CurrentPath.Children
            If child.Type = PathStructure.PathType.File Then
              If child.FileInfo.Extension.Contains(txtFileExtension.Text) Then
